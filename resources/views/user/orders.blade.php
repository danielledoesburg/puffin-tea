        ...
        <div class="accordion-item">
            <h2 class="accordion-header" id="headingOne">
                <button class="accordion-button" type="button" data-bs-toggle="collapse"
                    data-bs-target="#order{{ $order->id }}" aria-expanded="true" aria-controls="collapseOne">
                    <li><span class="boldorder">ordernr: </span>{{ $order->ordernr }}, </li>
                    <li> <span class="boldorder">order date: </span> {{ $order->created_at }} </li>
                </button>
            </h2>
            <div id="order{{ $order->id }}" class="accordion-collapse collapse" aria-labelledby="headingOne"
                ...
                <div class="accordion-body">
                    ...
                    @foreach($order->orderDetails as $orderDetails)
                        <li>{{ $orderDetails->quantity }}x {{ $orderDetails->unit_amount }}
                            {{ $orderDetails->unit }} of <strong>{{ $orderDetails->product }}</strong> for
                            &euro;{{ $orderDetails->price }} </li>
                    @endforeach
